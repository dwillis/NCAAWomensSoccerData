...
urls <- read_csv("url_csvs/ncaa_mens_soccer_teamurls_2025.csv") %>% pull(3)
...
matchstatsfilename <- paste0("data/ncaa_mens_soccer_matchstats_", season, ".csv")
...
for (i in urls){
  ...
  teamside <- matches %>% filter(opponent != "Defensive Totals" & opponent != "Goalkeeping")

  opponentside <- matches %>% filter(opponent == "Defensive Totals") %>% select(-opponent, -home_away, -overtime, -games) %>% rename_with(.cols = 6:16, function(x){paste0("defensive_", x)})

  # Extract goalkeeper stats if they exist in the table
  goalkeeperside <- matches %>% filter(opponent == "Goalkeeping")

  if(nrow(goalkeeperside) > 0) {
    goalkeeperside <- goalkeeperside %>%
      select(-opponent, -home_away, -overtime, -games) %>%
      rename_with(.cols = 6:ncol(.), function(x){paste0("goalie_", x)})
  }
  ...
  if(nrow(goalkeeperside) > 0) {
    joinedmatches <- left_join(joinedmatches, goalkeeperside, by = c("date", "team", "outcome", "team_score", "opponent_score"))
  }
