...
def parse_matches(page):
    ...
    # Column name mapping to match R output
    col_map = {
        "shatt": "sh_att",
        "sog": "so_g",
    }
...
def process_team(page, url):
    ...
    # Define stat columns
    stat_cols = [
        "gp", "minutes", "goals", "assists", "points", "sh_att", "so_g",
        "fouls", "red_cards", "yellow_cards", "corners", "pk", "pk_att", "gwg",
    ]
    ...
    def_stat_cols = [c for c in stat_cols if c in defside.columns and c != "gp"]
    ...
    numeric_cols = [c for c in joined.columns
                    if c not in ("date", "team", "opponent", "home_away", "outcome",
                                 "overtime", "minutes", "defensive_minutes")]
...
def main():
    ...
    urls_file = f"url_csvs/ncaa_mens_soccer_teamurls_{season}.csv"
    output_file = f"data/ncaa_mens_soccer_matchstats_{season}.csv"
